                ...
                async with session.ws_connect(WS_URL, headers={
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            if 'result' in data and 'prices' in data['result']['data']:
                                ...
                                for price in prices:
                                    ...
                                    if price_change < -80 and volume_u_5m < 10000:
                                        logging.info(f"Unsubscribing from mint {mint} (price_change: {price_change}, volume_u_5m: {volume_u_5m}).")
                                    ...
                                    if volume_u_5m > 200000:
                                        ...
                                        if token not in tracked:
                                            ...
                                            print(message)  # Redirects to Telegram via TelegramConsoleRedirector
                                        else:
                                            # Compare with existing volume and send update if needed
                                            prev_volume = tracked[token]
                                            if volume_u_5m > prev_volume * 1.5:
                                                tracked[token] = volume_u_5m
                                                message = (
                            ...
                            logging.error(f"WebSocket error for mint {mint}: {msg.data}")
                            break
